...
import { Brackets, Copy, Check } from "lucide-react";
...
export const ArrayNode = memo(({ data }: { data: ArrayNodeData }) => {
    ...
    <div
      className={`relative px-4 py-3 rounded-lg border-2 shadow-md transition-all duration-200 min-w-[200px] group hover:shadow-lg ${
        data.isHighlighted
          ...
          : "bg-node-array-bg border-node-array-border"
      }`}
    >
      <Handle
        type="target"
        position={Position.Top}
        className="w-3 h-3 !bg-node-array"
      />
      
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-1">
          <Brackets className="w-4 h-4 text-node-array flex-shrink-0" />
